...
async function getArtworkRgb565(trackId, imageUrl) {
  const cacheKey = `artwork_rgb565:${trackId}:${ART_SIZE}`;
  const cached = await kv.get(cacheKey);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // raw mode: return binary RGB565 of the artwork directly (ESP32 reads straight into a buffer)
    if (wantArt === 'raw') {
      if (!imageUrl) return res.status(404).send('no artwork');
      const rgb565 = await getArtworkRgb565(track.id, imageUrl);
      res.setHeader('Content-Type', 'application/octet-stream'); // override the default json header set above
      res.setHeader('Content-Length', rgb565.length);
